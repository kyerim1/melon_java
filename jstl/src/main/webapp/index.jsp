...
<%--
	...
	DAO : Data Access Object - 데이터베이스에 접근하기위한 클래스
	DTO : Data Transfer Obejct - 데이터 저장 클래스
	
	예)  회원가입을위해  아이디,비번,이름, 주소,연락처 등을 입력하고
	     가입 버튼을 클릭
	     
	    1. 가입요청을 Controller가 받는다.
	    2. 가입 요청을 처리해줄 service를 실행 한다.
	    ...
	    2. 로그인 처리를 해줄수 있는 service를 실행 한다.
	    3. service에서 데이터베이스 작업을 수행하는 DAO실행
	    4. DAO실행 결과 아이디 비번일치하면 service에서 세션생성
	       일치 하지않으면 로그인 실패를 Controller에게 전달
	    5. Controller는 로그인 성공유무에따른 service의 결과를
	       사용자에게 보낸다.
