...
<%-- view.jsp 
	id 파라미터의 값을 board테이블의 board_id 컬럼으로 조회 하여 
...
<%
	...
	// id 파라미터 값 가져와서 정수로 변환
	int id = Integer.parseInt( request.getParameter("id") );

	String sql ="select * from board where board_id=?";
	
	Board data=null; // select sql문 의 결과는 Board클래스 객체 하나에 저장되는 데이터
	...
	try{
		...
		if( db.rs.next() ){
			data = new Board( db.rs.getInt("board_id"), db.rs.getString("writer"),
